...
/**
 * An abstract base class for writing applets that simulate programs that use TextIO.
 ...
 * any static methods, static member variables, and static nested classes in your class.  (It's not
 * necessary to make final static member variables non-static.  Declare your class to be a
 * sub-class of TextIOApplet (and probably change its name).  Your class can then be used for
 * making applets on web pages, and the applet will have the same functionality as the class.
 * (Note that you have to include on your web site all the .class files that are created when 
 * TextIOApplet is compiled, along with the .class files from your own class.  Since this file
 ...
 * sub-classes so that they actually do IO in the applet instead of using the standard IO streams.
...
abstract public class TextIOApplet extends JApplet {
    ...
    * the real System class by using its full name, "java.lang.System".)
    */
    ...
    * is doing a long computation during which it does no IO, it will not automatically detect
    * the abort flag.  If you want the program to respond to aborts during such computations,
    ...
    * in any case, if it were thrown by an applet.)
    */
   ...
   private class Console extends JPanel {
      ...
      void checkAbort() {
         if (aborted && Thread.currentThread() == runner)
